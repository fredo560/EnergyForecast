...
def fetch_lmp_prices():
    ...
    df["hour"] = df["Time"].dt.tz_localize(None)  # Remove timezone for comparison
    ...
    # Split data
    df1 = df_combined[df_combined["date"] <= now_naive].copy()
    df2 = df_combined[df_combined["date"] > now_naive].copy()
