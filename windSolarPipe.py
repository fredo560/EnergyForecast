...
def fetch_wind_solar():
    ...
    print(df.head(24))
    print(df.columns)
    df['datetime'] = pd.to_datetime(df['interval_start_utc'],errors="coerce",utc=True).dt.tz_convert(houston_tz) #.dt.tz_localize(None)  # Convert to Houston local time
...
def fetch_wind_solar_forecast(days_ahead=8):
    ...
    df['date'] = pd.to_datetime(df['interval_start_utc'],errors="coerce",utc=True).dt.tz_convert(houston_tz).dt.floor("h")
